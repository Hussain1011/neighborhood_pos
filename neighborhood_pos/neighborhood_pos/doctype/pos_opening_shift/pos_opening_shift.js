...
frappe.ui.form.on('POS Opening Shift', {
	setup(frm) {
		...
		frm.set_query("user", function(doc) {
			return {
				query: "neighborhood_pos.neighborhood_pos.doctype.pos_closing_shift.pos_closing_shift.get_cashiers",
	...
	add_opening_cash: function(frm){
		frm.call({
			"method": "neighborhood_pos.neighborhood_pos.doctype.pos_opening_shift.pos_opening_shift.check_opening_cash",
			"args": {
					...
					var d = new frappe.ui.Dialog({
						...
						primary_action: function () {
							return frm.call({
								...
								freeze: true,
								method: "neighborhood_pos.neighborhood_pos.doctype.pos_opening_shift.pos_opening_shift.add_opening_cash",
								callback: function (r) {
									if (r.exc) {
	...
	change_status_to_open: function(frm) {
		frm.call({
			"method": "neighborhood_pos.neighborhood_pos.doctype.pos_opening_shift.pos_opening_shift.change_status_to_open",
			"args": {
				"pos_opening_entry": frm.doc.name
			},
			freeze: true,
