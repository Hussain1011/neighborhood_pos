  ...
  methods: {
    ...
    submit_invoice(print) {
      ...
      data["is_cashback"] = this.is_cashback;
          ...
          const vm = this;
          frappe.call({
            method: "neighborhood_pos.neighborhood_pos.api.posapp.submit_invoice",
            args: {
              data: data,
          invoice: this.invoice_doc,
          taxvalue: this.taxType || "",
            },
            async: true,
            callback: async function (r) {
              if (r.message) {
                ...
                console.log("submitt...called", r.message);
                if (print) {
                  try {
    ...
    set_full_amount(idx) {
      this.invoice_doc.payments.forEach((payment) => {
        if (payment.idx === idx) {
          ...
          if (!this.pos_profile.tax_not_included) {
            let getTax = this.pos_profile.payments.find((item) => {
              return payment.mode_of_payment === item.mode_of_payment;
            });
            console.log("gettax", getTax);
            this.taxType = getTax.tax_and_charges;
            ...
            //   taxRate = 0.05;
            //   const subtotal = this.invoice_doc.net_total;
            //   const taxAndCharges = subtotal * taxRate;
            //   this.invoice_doc.total_taxes_and_charges = taxAndCharges;
            //   this.invoice_doc.rounded_total = Math.ceil(
            //     taxAndCharges + this.invoice_doc.net_total
            //   );
    ...
    get_available_credit(e) {
      ...
      if (e) {
        frappe
          .call("neighborhood_pos.neighborhood_pos.api.posapp.get_available_credit", {
    ...
    get_addresses() {
      ...
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_customer_addresses",
    ...
    get_sales_person_names() {
      ...
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_sales_person_names",
        callback: function (r) {
          if (r.message) {
    ...
    request_payment() {
      ...
      frappe
        ...
        .then(() => {
          frappe
            .call({
              method: "neighborhood_pos.neighborhood_pos.api.posapp.create_payment_request",
              args: {
                doc: vm.invoice_doc,
              },
            })
    ...
    get_mpesa_modes() {
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.m_pesa.get_mpesa_mode_of_payment",
  ...
  mounted: function () {
    this.$nextTick(function () {
      evntBus.$on("send_invoice_doc_payment", (invoice_doc) => {
        let taxtype = this.pos_profile.payments.find((item) => {
          if (item.default) {
            return item;
          }
        });
        this.taxType = taxtype.taxes_and_charges;
