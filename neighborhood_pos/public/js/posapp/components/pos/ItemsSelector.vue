      ...
      <v-row class="items px-2 py-1">
        <v-col class="pb-0 mb-2">
          <v-text-field
            ...
            @keydown.enter="search_onchange(true)"
            ref="debounce_search"
          ></v-text-field>
        </v-col>
              ...
              >
                <v-card hover="hover" @click="add_item(item)">
                  <v-img
                    :src="
                      item.image ||
                      '/assets/neighborhood_pos/js/posapp/components/pos/placeholder-image.png'
                    "
      ...
      <v-row no-gutters align="center" justify="center">
        <v-col cols="12">
          <v-select
            ...
            hide-details
            v-model="item_group"
            v-on:change="search_onchange(false)"
          ></v-select>
        </v-col>
...
export default {
  ...
  watch: {
    ...
    ordertype() {
      // if(this.pos_profile.pos_theme == 'Default'){
      this.get_items();
      // }
    },
  },
  ...
  methods: {
    ...
    get_items() {
      ...
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_items",
        args: {
    ...
    get_items_groups() {
      ...
      if (this.pos_profile.item_groups.length > 0) {
        ...
        const vm = this;
        frappe.call({
          method: "neighborhood_pos.neighborhood_pos.api.posapp.get_items_groups",
          args: {},
          callback: function (r) {
            if (r.message) {
    ...
    add_item(item) {
      ...
      if (item.has_variants) {
        this.get_variants(item, this.items);
    ...
    get_variants(value, values) {
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_variants_addons",
        args: {
        ...
        callback: function (r) {
          if (r.message) {
            console.log("get_variants", r.message);
            evntBus.$emit("open_variants_model", value, values, r.message);
          }
        },
      });
    },
        ...
        callback: function (r) {
          if (r.message) {
            items.forEach((item) => {
              ...
              item.add_ons = updated_item.add_ons;
            });
          }
        },
      });
  ...
  computed: {
    filtred_items() {
      ...
      if (!this.pos_profile.pose_use_limit_search) {
        ...
        if (this.pos_profile.search_specific) {
          if (
            this.pos_profile.posa_show_template_items &&
            this.pos_profile.posa_hide_variants_items
          ) {
            console.log("enter in 2", filtred_list);
            if (this.search.length == 1) {
              console.log("enter in if");
              const data = filtred_list
                .filter((item) => !item.variant_of)
                .slice(0, 50)
                .reverse();
              console.log("data", data);
              let filterd = data.filter((item) => {
                return item.item_code == this.search;
              });
              return filterd;
            } else {
            ...
            console.log("enter in 3", filtred_list);
            // return filtred_list.slice(0, 50).reverse();
            if (this.search.length == 1) {
              const data = filtred_list
                .filter((item) => !item.variant_of)
                .slice(0, 50)
        ...
        return this.items.slice(0, 50).reverse();
      }
    },
    debounce_search: {
      get() {
  ...
  mounted() {
    this.scan_barcoud();
    evntBus.$on("send_order_type", (data) => {
      this.ordertype = data?.name;
    });
    evntBus.$on("update_get_item", (data) => {
      this.ordertype = data?.name;
      // this.get_items()
    });
  },
