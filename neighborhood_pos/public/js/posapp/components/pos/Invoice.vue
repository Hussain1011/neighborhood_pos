<template>
  <div>
    ...
    >
      ...
      <v-row
        align="center"
        class="items px-2 py-3"
        v-if="pos_profile.allow_table_no"
      >
        <v-col cols="3" class="pb-2 mb-0 pr-0 pt-0">
          <v-select
            dense
            hide-details
            outlined
        ...
        <v-col
          cols="3"
          class="pb-2 mb-0 pr-0 pt-0"
          v-if="ordertype.order_type !== 'TAKE AWAY'"
        >
          <div>
            <v-autocomplete
              dense
        ...
        <!-- <v-col cols="3" class="mb-0 pr-0 pt-0 mt-7">
          <div class="">
            <v-text-field
              dense
              outlined
...
export default {
  ...
  data() {
    return {
      ...
      invoiceType: "Invoice",
      orderTypes: [], //"A LE CART", "TAKE AWAY", "BUFFET"
      ordertype: "",
      tableOptions: [], // Array to hold the dropdown options
      selectedTableStatus: "",
      selectedTable: null, // Variable to store the selected option
      itemsPerPage: 1000,
      expanded: [],
      singleExpand: true,
      cancel_dialog: false,
  ...
  methods: {
    handleChange() {},
    changeOrderType() {
      evntBus.$emit("update_get_item", this.ordertype);
    },
    remove_item(item) {
    ...
    updateQuantity(item) {
      item.qty = parseFloat(item.amount) / parseFloat(item.rate);
    },
    updateAmount(item) {
      item.amount = parseFloat(item.qty) * parseFloat(item.rate);
    },
    add_one(item) {
      item.qty++;
      if (item.qty == 0) {
    ...
    cancel_invoice() {
      ...
      if (doc.name && this.pos_profile.posa_allow_delete) {
        frappe.call({
          method: "neighborhood_pos.neighborhood_pos.api.posapp.delete_invoice",
    ...
    new_invoice(data = {}, key) {
      ...
      if (!data.name && !data.is_return) {
        ...
        this.selectedTable = data.table_no;
        if (data?.resturent_type) {
          let type = this.orderTypes.filter((item) => {
            return item.name === data.resturent_type;
          });
    ...
    get_invoice_doc() {
      ...
      doc.table_no = this.selectedTable;
    ...
    async get_invoice_from_order_doc() {
      ...
      if (this.invoice_doc.doctype == "Sales Order") {
        await frappe.call({
          method:
            "neighborhood_pos.neighborhood_pos.api.posapp.create_sales_invoice_from_order",
          args: {
    ...
    update_invoice(doc) {
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.update_invoice",
        args: {
          data: doc,
        },
        async: false,
    ...
    update_invoice_from_order(doc) {
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.update_invoice_from_order",
        args: {
          data: doc,
        },
        async: false,
    ...
    async show_payment() {
      ...
      if (this.invoice_doc.doctype == "Sales Order") {
        ...
        invoice_doc.additional_discount_percentage = Number(
          this.additional_discount_percentage
        );
        ...
        frappe.call({
          method:
            "neighborhood_pos.neighborhood_pos.api.posapp.get_sales_invoice_child_table",
          args: {
    ...
    get_draft_invoices() {
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_draft_invoices",
        args: {
    ...
    get_draft_orders() {
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.search_orders",
        args: {
    ...
    update_items_details(items) {
      ...
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_items_details",
        async: false,
        args: {
    ...
    update_item_detail(item) {
      ...
      const vm = this;
      frappe.call({
        method: "neighborhood_pos.neighborhood_pos.api.posapp.get_item_detail",
        args: {
    ...
    fetch_customer_details() {
      ...
      if (this.customer) {
        frappe.call({
          method: "neighborhood_pos.neighborhood_pos.api.posapp.get_customer_info",
          args: {
    ...
    async generate_pre_invoice_doc() {
      let doc = {};
      let vm = this;
      await frappe.call({
        method:
    ...
    async print_draft_invoice(key) {
      if (key && this.saleOrder) {
        await this.generate_pre_invoice_doc();
      }
      ...
      frappe.run_serially([
        ...
        () => {
          this.load_print_page(invoice_name, key);
        },
      ]);
    },
    set_delivery_charges() {
      ...
      frappe.call({
        method:
          "neighborhood_pos.neighborhood_pos.api.posapp.get_applicable_delivery_charges",
        args: {
          company: this.pos_profile.company,
          pos_profile: this.pos_profile.name,
          customer: this.customer,
    ...
    async fetchTableOptions() {
      try {
        ...
        this.tableOptions = response.message;
      } catch (error) {
  ...
  mounted() {
    evntBus.$on("register_pos_profile", (data) => {
      console.log("register_pos_profile", data);
    ...
    evntBus.$on("load_invoice", (data) => {
      console.log("reserved table", data);
      this.customer = data.title;
      this.new_invoice(data, false);
    ...
    evntBus.$on("flag-for-sale-order", (data) => {
      this.saleOrder = data;
    });
  },
  beforeDestroy() {
  ...
  watch: {
    additional_discount_percentage: {
      handler(newVal) {
        console.log("watcher calling...", newVal);
        this.invoice_doc.additional_discount_percentage = newVal;
      },
      deep: true,
    },
    selectedTable(newVal) {
      if (newVal) {
        const selected = this.tableOptions.find((item) => item.name === newVal);
        if (selected) {
          this.selectedTableStatus = selected.status;
        }
      } else {
        this.selectedTableStatus = "";
      }
    },
    customer() {
      this.close_payments();
