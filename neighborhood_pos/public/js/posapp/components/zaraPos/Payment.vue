    ...
    <v-card class="payment-card py-4 px-8">
        ...
        <v-col cols="12" md="7" class="d-flex">
          ...
          >
            <v-btn
              ...
              variant="outlined"
              size="large"
              class="text-capitalize b-radius-8 ml-2"
      ...
      <v-row class="mt-2 pb-0">
        ...
        <v-col cols="12" md="1" v-show="splitPayment">
          <v-btn
            class="mr-8 b-radius-8 split-close"
            color="#21A0A0"
            size="large"
            variant="outlined"
            style="background-color: #d3ecec"
      ...
      <v-col cols="12">
        <v-card class="keyboard-card">
          <div class="text-center">
            <v-row>
              <v-col
                v-for="(button, index) in numpad"
                :key="index"
                cols="4"
                class="py-1"
              >
                <v-btn
                  class="mx-2 text-capitalize b-radius-8"
                  variant="outlined"
                  color="#21A0A0"
                  block
                  style="background-color: #d3ecec"
        ...
        <v-card-text>
          ...
          <v-row>
            <v-col v-for="(mode, index) in paymentModes" :key="index" cols="12">
              ...
              <v-text-field
                v-model="mode.amount"
                :label="`Enter amount for ${mode.mode_of_payment}`"
                type="number"
                outlined
      ...
      if (
        ...
        try {
          const response = await frappe.call({
            method: "neighborhood_pos.neighborhood_pos.api.posapp.sales_invoice",
            args: {
              data: data,
